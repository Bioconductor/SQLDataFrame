...
```{r}
dbfile <- system.file("extdata/test.db", package = "SQLDataFrame")
ss <- SQLDataFrame(dbname = dbfile, dbtable = "state", dbkey = "state")
## ss1 <- ss[1:10, 2:3]
## ss1
dbname(ss)
dbtable(ss)
...
```{r}
identical(as.data.frame(ss11@tblData), as.data.frame(ss21@tblData))
```
...
### Numeric calculation / Data manipulation (to-be-updated!)
