...
whole table into the other connection. Use of advanced functions or
debugging often involves specialized SQL knowledge which brings
challenges for common _R_ users.
...
grammar by supporting, such as, `%>%`, `select`, `filter`, `mutate`,
etc. More importantly, special SQL-based strategies were implemented
in SQLDataFrame to efficiently handle the cross-database operations
...
operations are made possible in _R_, such as cross-database ID
matching and conversion, variant annotation extraction, etc. The
scalability and interoperability for SQLDataFrame with other
bioinformatics tools are expected to significantly promote the gaining
of biological insights.

Currently `SQLDataFrame` supports the DBI backend of `RSQLite` and
`MySQL`. In the future or upon feature request, we would implement
this package so that users could choose to use different database
backend when doing the `SQLDataFrame` constructor and
`saveSQLDataFrame`.  
...
SQLDataFrame now supports the MySQL database tables, for local MySQL
servers, or remote ones on the web or cloud. The SQLDataFrame
construction, `*_join` functions, `union`, `rbind`, and saving are all
supported. Aggregation operations are supported for same or cross
MySQL databases. Details please see the function documentations.
