...
7. tryCatch: Error in result_create(conn@ptr, statement) : parser stack overflow. -- show, `.join_union_prepare`.
8. as.data.frame(), show(), extra layer of query? 
10. *_join no S4 generics. -- Keep S3 method. 
...
12. Should `overwrite = FALSE` check `dbname` or `dbtable`? (in `makeSQLDataFrame()` and
