...
## todo: 
1. tidy code for `joinSQLDataFrame.R` to avoid duplicate code. -- done!
2. `union()@dbconcatKey` not efficient.
save @dbconcatKey as a data.frame, which contains same information as the SQL index. (add index file for SQLDataFrame constructor if not already exists!)
3. Try real database with different SQL format. (MySQL, BigQuery, ...)
4. write `readSQLtable()` function, with ordinary format (.csv, .txt...)
   as input, write database with optional SQLite format, open as
   SQLDataFrame. (`dbWriteTable(con, name, value = "filename")`)
5. `apply()`?? (e.g., print...) 
6. coerce into matrix for `row/colSums`, etc. 
7. tryCatch: Error in result_create(conn@ptr, statement) : parser stack overflow
8. as.data.frame(), show(), extra layer of query? 
9. BiocGenerics:::replaceSlots()
