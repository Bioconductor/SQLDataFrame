...
## todo: 
- add "union" method for SDF, which returns union with unique rows and maintain the original order.  -- done!
- modify the "state" table, for unique "region+population". 
- modify `dbtable()` accessor for `op_distinct` or other ops...
- reimplement "rbind" method, which extend "union" and update the @indexes. 
