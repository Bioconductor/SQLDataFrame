...
- "[,SQLDataFrame" always keep key column in @indexes[[2]]. 
  - save match(dbkey, colnames(object))
  - `colnames(SQLDataFrame)` still shows original colnames. 
  - `[i,j]`, "j" should correspond to the original col orders. 
  - update `@includeKey` if `j` doesn't include `which(key)`.
- "show, SQLDataFrame" Always show key column as first column. e.g.,
  ...
  - `.printROWS`, print "key | otherCol1, otherCol2, ..."

- remove "rownames", from constructor/accessor/...

- to always have key() column available, could be used for selecting
  rows, but works like an attribute, not an explicit column, do not
