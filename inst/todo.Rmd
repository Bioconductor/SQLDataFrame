## todo: 
- create index table in SQL database? 
- rewrite the "union" method for `@dbconcatKey`. Not efficient now! 
...
- leave the current `dbtable()` method as is. When determined with the
  above, disable the `dbtable()` for non-`op_base_xx` SDF objects. -- done!
